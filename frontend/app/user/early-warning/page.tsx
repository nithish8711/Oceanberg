...
import { LeafletMap } from "@/components/maps/leaflet-map" // Assuming this is the map component
...
const ALERTS_BASE =
  (typeof process !== "undefined" && process.env.NEXT_PUBLIC_ALERTS_API) ||
...
type Alert = {
  ...
  district: string
  state: string
  color: "Yellow" | "Orange" | "Red" | "GREEN" // Added "GREEN" based on your API response
  message: string
  source: string
  issueDate: string
  latitude: number
...
export default function EarlyWarningPage() {
  ...
  const { data, isLoading } = usePersistentSWR<Alert[]>(
    ...
    async ([base, f]: [string, typeof filters]) => {
      ...
      // Updated logic for district/state to handle "all" case properly
  ...
  // Authorities Alerts (local filter by source)
  ...
  // Local Search Filter (searchedAlerts equals all authoritativeAlerts when q is empty)
  const searchedAlerts = useMemo(() => {
    const term = q.trim().toLowerCase()
    if (!term) return authoritativeAlerts // Shows ALL if search is empty
  ...
  // Map Markers logic: CRITICAL FIX for displaying only searched results
  const markers = useMemo(
    () =>
      // Use searchedAlerts so the map updates with the search results
      searchedAlerts.map((a, idx) => ({
        ...
        color:
          a.color === "Red"
            ...
            : a.color === "Yellow"
            ? "#ca8a04"
            : "#2563eb", // Fallback blue for "GREEN" and others
        popup: `${a.message}\n${a.district}, ${a.state}\n${new Date(a.issueDate).toLocaleString()}`,
      })),
    [searchedAlerts], // Dependency ensures update on search/filter change
  )
  ...
  useEffect(() => {
    ...
    const newest = Math.max(
      ...alerts.map((a) => new Date(a.issueDate).getTime()).filter((x) => !Number.isNaN(x)),
      0,
    )
    ...
    <main className="mx-auto w-full max-w-6xl p-4 md:p-6">
        ...
        <Card className="md:col-span-3">
          ...
          <CardContent className="p-0">
                ...
                <Popover>
                  ...
                  >
                    <AlertFilters
                      ...
                      // Pass unique values from authoritative alerts for filter options
                      types={[...new Set(authoritativeAlerts.map((a) => a.type))]}
                      colors={["Yellow", "Orange", "Red", "GREEN"].filter(c => authoritativeAlerts.some(a => a.color === c))}
            ...
            <div className="relative z-0 h-[420px]">
              {/* The map now correctly receives the markers array based on the search/filter results */}
      ...
      <section className="space-y-4">
        <div className="space-y-3">
          {/* List displays the searched alerts */}
